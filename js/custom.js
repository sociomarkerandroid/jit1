...
jQuery(document).ready(function($){
	...
	$('.mainMenuButton').click(function(){
		...
		if(mainMenuStatus == 'closed' && mainMenuAnimation == 'complete'){
			...
			$('.mainMenuWrapper').stop(true, true).animate({"margin-top": 0}, 500, 'easeOutQuart', function(){mainMenuStatus = 'open'; mainMenuAnimation = 'complete';});
			
		}else if(mainMenuStatus == 'open' && mainMenuAnimation == 'complete'){
			
			mainMenuAnimation = 'incomplete';
			$('.mainMenuWrapper').stop(true, true).animate({"margin-top": -mainMenuHeight}, 500, 'easeInQuart', function(){mainMenuStatus = 'closed'; mainMenuAnimation = 'complete'; $('.mainMenuWrapper').css('display', 'none'); $('body, html').stop(true, true).animate({scrollTop:0}, 1000,'easeOutCubic');});
	...
	function adaptMenuItems(){
		...
		if($(window).width() >= 420){
			...
			var menuItemWidth = ($('.mainMenuWrapper').width() - 48 - 12 - 24)/2;
			$('.mainMenuWrapper > li').css('width', menuItemWidth);
		
		} else {
			
			$('.mainMenuWrapper > li').removeAttr('style');
